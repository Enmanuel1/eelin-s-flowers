    ...
    <button className={'flower-button ' + (bloomed ? 'is-bloomed' : '')} type="button" onClick={() => !bloomed && onBloom()} aria-label={bloomed ? 'La flor amarilla ha florecido' : 'Toca para hacer florecer la flor amarilla'}>
      <motion.span className="flower-sway" animate={reduceMotion ? {} : { rotate: [0, 1.15, 0, -1.15, 0], y: [0, -3, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}>
        <span className="flower">
          ...
          <span className="petal-ring outer-ring" aria-hidden="true">{outerPetals.map((_, index) => <span className={'petal outer-petal tone-' + (index % 3)} style={{ '--angle': (index * 20) + 'deg', '--delay': (index * .05) + 's' }} key={index} />)}</span>
          <span className="petal-ring inner-ring" aria-hidden="true">{innerPetals.map((_, index) => <span className={'petal inner-petal tone-' + (index % 3)} style={{ '--angle': (index * (360 / 14) + 10) + 'deg', '--delay': (.48 + index * .055) + 's' }} key={index} />)}</span>
